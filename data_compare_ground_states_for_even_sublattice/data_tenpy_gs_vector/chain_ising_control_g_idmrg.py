...
def example_DMRG_tf_ising_infinite(gzz,gx,gzxz):
    print("infinite DMRG, transverse field Ising model")
#    print("g={g:.2f}".format(g=g))
    print("gzz={gzz:f}".format(gzz=gzz))
    print("gx={gx:f}".format(gx=gx))
    print("gzxz={gzxz:f}".format(gzxz=gzxz))
#    model_params = dict(L=2, gzz=2.0*(1.0-g**2), gx=(1.0+g)**2, gzxz=(g-1.0)**2, bc_MPS='infinite', conserve=None)
    L = 2
    model_params = dict(L=L, gzz=gzz, gx=gx, gzxz=gzxz, bc_MPS='infinite', conserve=None)
...
#    product_state = (["up", "down"] * (M.lat.N_sites))[:M.lat.N_sites] ## https://tenpy.johannes-hauschild.de/viewtopic.php?t=30
    ...
    mx = np.mean(psi.expectation_value("Sigmax"))
    mz = np.mean(psi.expectation_value("Sigmaz"))
## https://tenpy.readthedocs.io/en/latest/reference/tenpy.networks.mps.MPSEnvironment.html#tenpy.networks.mps.MPSEnvironment.expectation_value_multi_sites
    mstring = np.mean([psi.expectation_value_multi_sites(["Sigmaz","Sigmay","Sigmax","Sigmay","Sigmaz"],i0) for i0 in range(L)])
#    print(mstring)
...
#
    parser.add_argument("-Lfinite",metavar="Lfinite",dest="Lfinite",type=int,default=4,help="set Lfinite")
#
...
#
#    parser.add_argument("-gzxz",metavar="gzxz",dest="gzxz",type=np.float64,default=0.0625,help="set gzxz")
#    parser.add_argument("-gzz",metavar="gzz",dest="gzz",type=np.float64,default=0.875,help="set gzz")
#    parser.add_argument("-gx",metavar="gx",dest="gx",type=np.float64,default=3.0625,help="set gx")
#
...
#        dat.append([g,E,max(psi.chi),psi.correlation_length()])
    E, psi, M, mx, mz, mstring = example_DMRG_tf_ising_infinite(gzz,gx,gzxz)
    print(gzz,gx,gzxz,E,max(psi.chi),psi.correlation_length(),mx,mz,mstring)
    dat.append([gzz,gx,gzxz,E,max(psi.chi),psi.correlation_length(),mx,mz,mstring])
#
## print MPS
...
##            "parameters": {"L": L, "g": g}
##            "parameters": {"g": g}
#            "parameters": {"gzz": gzz, "gx": gx, "gzxz": gzxz}
#            }
#        with h5py.File("dat_g"+"{:.10f}".format(g)+".h5",'w') as f:
#            hdf5_io.save_to_hdf5(f,datall)
#
#    np.savetxt("dat",dat,header="gzz,gx,gzxz,E,chi,corrlength,mx,mz,mstring")
    np.savetxt("dat_gzz"+"{:.10f}".format(gzz)+"_gx"+"{:.10f}".format(gx)+"_gzxz"+"{:.10f}".format(gzxz),dat,header="gzz,gx,gzxz,E,chi,corrlength,mx,mz,mstring")
#
## https://github.com/tenpy/tenpy/blob/main/doc/intro/npc.rst
#    print(psi.get_B(0)._data[0])
#    print(psi.get_B(1)._data[0])
#    print(psi.get_B(0)._qdata)
#    print(psi.get_B(1)._qdata)
#
## prepare a finite-size ground-state vector from infinite MPS
#    Lfinite = 4
    mpss = []
    for i in range(Lfinite):
        mpss.append(psi.get_B(i%2)._data[0])
    ## MPS to rank-L tensor-type wave function
    vecLrank_ = mpss[0]
    for i in range(2,Lfinite):
        vecLrank_ = np.tensordot(vecLrank_,mpss[i-1],axes=([i],[0]))
    vecLrank_ = np.tensordot(vecLrank_,mpss[Lfinite-1],axes=([Lfinite,0],[0,2]))
    #
    vec = vecLrank_.reshape(1,-1)[0] ## rank-L tensor to vector
    vec /= np.linalg.norm(vec) ## normalize vector
    vec *= np.sign(vec[0].real) ## set a first component of a vector positive
    vecLrank = vec.reshape(Lfinite*[2]) ## vector to rank-L tensor
#    vec2 = vecLrank.reshape(1,-1)[0] ## rank-L tensor to vector
#    print(vec)
#    print(vecLrank)
#    print(vec2)
#    print(np.array_equal(vec,vec2))  
#
    np.savetxt("dat_vec_L"+"{}".format(Lfinite),vec)
    np.savez("dat_vecLrank_L"+"{}".format(Lfinite),vecLrank)
